	...
	<div class="col">
		<div class="card">
			...
			<div class="card-body" style="margin: 20px;">
				<form method="post" action="/saledet/update/{{$nota_id}}/{{$product_id}}" id="catForm">
					@csrf
					<h6>ID Penjualan*</h6>
					<div class="row" style="margin-bottom: 20px">
						<div class="col-9">
							<select class="selectpicker" data-live-search="true" name="nota_id" required>
								...
								@foreach($sales as $c)
									@if(($c->nota_id) == $nota_id)
									<option value="{{$c -> nota_id}}" selected>{{$c -> nota_id}}</option>
									@else
					...
					<div class="row">
						...
						<select class="selectpicker" data-live-search="true" name="product_id" required style="width: 100%;">
								...
								@foreach($products as $c)
									...
									<option value="{{$c -> product_id}}" selected>{{$c -> product_name}}</option>
									@else
						...
						<div class="col-2">
						    <label class="mdc-text-field mdc-text-field--outlined" style="width: 100%;">
							  <input type="number" class="mdc-text-field__input" aria-labelledby="my-label-id" name="quantity" required onkeypress="return numOnly(event)" max="99" min="1" value="{{ $saledet->quantity }}">
						...
						<div class="col-4">
						    <label class="mdc-text-field mdc-text-field--outlined" style="width: 100%;">
							  <input id="currency" type="text" class="mdc-text-field__input" aria-labelledby="my-label-id" name="discount" required onkeypress="return numOnly(event)" maxlength="16" value="{{ $saledet->discount }}">
							  <div class="mdc-notched-outline">
							   <div class="mdc-notched-outline__leading"></div>
							    <div class="mdc-notched-outline__notch">
